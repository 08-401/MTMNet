...
class AverageMeter:
    ...
    def __init__(self, dataset, device='cuda'):
        ...
        if self.benchmark == 'pascal':
            ...
            self.class_ids_interest = torch.tensor(self.class_ids_interest).to(device)
            self.nclass = 1
        elif self.benchmark == 'wbc':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        elif self.benchmark == 'kvasir':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        elif self.benchmark == 'tcia':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        else:
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        # Log arguments
        logging.info('\n:==== MTMNet: Multi-task Learning with Minimal Data for Source-free Cross-domain Few-shot Semantic Segmentation =====')
